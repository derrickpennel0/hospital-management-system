...
module.exports.getUserType = function(userid, callback){
  	var query = "SELECT login.username, employee.type FROM employee INNER JOIN login ON employee.id = login.username AND login.username = '1000' WHERE EXISTS (SELECT * FROM login WHERE username = '1000');";
  	con.query(query, callback);	
}
